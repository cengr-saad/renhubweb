...
BEGIN
  ...
  FROM (
    ...
    WHERE 
      ...
      AND (p_search_query IS NULL OR p_search_query = '' OR 
           ...
           l.location ILIKE '%' || p_search_query || '%')
  ...
  WHERE 
    ...
    AND (p_sort_by NOT IN ('top_rated') OR sub.average_rating >= 4.5)
  ORDER BY 
    -- FEATURED FIRST
    ...
    CASE WHEN p_sort_by IN ('top_rated', 'top_rated_broader') THEN sub.average_rating END DESC,
    ...
    -- FALLBACK SORT (handles randomizing 0-rating items if needed)
    CASE WHEN p_sort_by IN ('top_rated_broader') AND sub.average_rating = 0 THEN random() END DESC
